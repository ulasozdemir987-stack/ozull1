 ...
 * Filmler/Diziler/Canlı is instant (the provider's first fetch is slow). Mounted
...
export function CatalogWarmer() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const firstDiziler = (seriesCats as Category[])[0]?.category_id;
      if (firstVod) qc.prefetchQuery({ queryKey: ["vod", "streams", firstVod], queryFn: () => api.vodStreams(firstVod) });
      if (firstDiziler) qc.prefetchQuery({ queryKey: ["series", "list", firstDiziler], queryFn: () => api.series(firstDiziler) });
